...
def build_telegram_ai_failure_text(client_info, final_score, ai_error):
    ...
    return (
        "ℹ️ Gemini временно недоступен; отчет собран экспертным движком BTG Audit\n"
        f"🏢 Компания: {client_info.get('Наименование компании', '-')}\n"
        f"📍 Город: {client_info.get('Город', '-')}\n"
        f"📊 Сфера: {client_info.get('Сфера деятельности', '-')}\n"
        f"🌐 Сайт: {client_info.get('Сайт компании', '-')}\n"
...
def render_app_header():
    ...
    logo_html = (
        f'<img src="{logo_uri}" alt="BTG Trade">'
        if logo_uri
        else '<strong>BTG Trade</strong>'
    )
    ...
    <div class="audit-hero audit-brand-hero">
        ...
        <div class="audit-logo-lockup">
            {logo_html}
            <div class="brand-name">BTG Audit System v11.01</div>
...
def build_draft_download():
    ...
    draft_filename = f"btg_audit_draft_{datetime.now():%Y%m%d_%H%M}.json"
    ...
    with st.container(key="floating_draft_save"):
        ...
        * {{
        ...
        async function shareDraft() {{
            ...
            const shareData = {{
                title: "BTG Audit System",
                text: "Заполненная анкета BTG Audit System by Ivan Rudoy",
                url: shareUrl
            }};
...
def render_generation_guard(active):
    ...
    if (!target.__btgBeforeUnloadHandler) {{
      target.__btgBeforeUnloadHandler = (event) => {{
    ...
    if ("{action}" === "install" && !target.__btgBeforeUnloadActive) {{
      target.addEventListener("beforeunload", target.__btgBeforeUnloadHandler);
      target.__btgBeforeUnloadActive = true;
    }}

    if ("{action}" === "remove" && target.__btgBeforeUnloadActive) {{
      target.removeEventListener("beforeunload", target.__btgBeforeUnloadHandler);
      target.__btgBeforeUnloadActive = false;
    }}
    </script>
    """, height=0)
...
if st.session_state.generation_state == "preparing":
    ...
    st.info("⚙️ `[СИСТЕМА]`: Инициализация аналитического ядра BTG Consulting v11.01...")
